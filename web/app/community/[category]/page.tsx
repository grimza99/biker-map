...
export default async function CommunityCategoryPage({
  ...
  const posts = getCommunityPostsByCategory(categoryMeta.slug as CommunityCategorySlug);
      ...
      <div className="grid gap-5 rounded-[26px] border border-border bg-panel/82 p-6 shadow-[0_18px_50px_rgba(5,6,7,0.24)] backdrop-blur-xl md:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="grid gap-3">
            <p className="m-0 text-[13px] font-semibold uppercase tracking-[0.08em] text-accent">커뮤니티 카테고리</p>
            <h1 className="m-0 text-[clamp(28px,4vw,42px)] font-semibold tracking-[-0.04em] text-text">
              {categoryMeta.label}
            </h1>
            <p className="max-w-[62ch] text-sm leading-7 text-muted">{categoryMeta.hint} 기준의 글 목록입니다.</p>
          </div>
          ...
          {posts.map((post) => (
            <article
              ...
              className={cn(
                ...
                post.pinned ? "border-accent/30 bg-accent/5" : "border-border bg-bg/52"
              )}>
