...
import { type AuthActionState, loginAction, signUpAction } from "../actions";
import {
  type AuthTabValue,
  loginFormSchema,
  signUpFormSchema,
    ...
    >
      ...
      <TabsContent value="login">
        <DefaultCardContainer className="gap-5 rounded-[28px] p-6 md:p-7">
          <h2 className="text-center m-0 text-2xl font-semibold tracking-(--tracking-heading-md) text-text">
            바이커맵 로그인
          </h2>
          ...
          <form className="grid gap-4" onSubmit={submitLogin} noValidate>
            ...
            {loginState.serverError ? (
              <p className="m-0 rounded-2xl border border-danger/30 bg-danger/8 px-4 py-3 text-sm text-danger">
                {loginState.serverError}
              </p>
            ) : null}
